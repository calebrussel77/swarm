...
const cardInformationSchema = z.object({
  cardNumber: z
    ...
    .max(16)
    .describe(
      "The number of the credit card to validate. Must be exactly 15 or 16 numerical digits"
    ),
  cardExpirationMonth: z
    ...
    .max(12)
    .describe(
      "The month of the year, described as a number where January is 1 and December is 12"
    ),
  cardExpirationYear: z
    .number()
    .min(2024)
    .max(2036)
    .describe("The year that the credit card expires in"),
  cardCvvCode: z
    .number()
    .min(100)
    .max(9999)
    .describe("The 3- or 4-digit numerical CVV / security code for the card"),
...
const bankAccountInformationSchema = z.object({
  accountNumber: z
    .string()
    .describe(
      "The bank account number provided by the user; comes in varying lengths and formats"
    ),
  routingNumber: z
    .string()
    .describe(
      "The routing number provided by the user, comes in varying formats and lengths"
    ),
  accountHolder: z
    .string()
    .describe(
      "The full name of the individual or business that owns the account"
    ),
});

const productSchema = z
  .enum(["IBM Watson API", "IBM Transcription API"])
  .describe(
    "The product that the charge is for. pricing will be calculated automatically."
  );

const authenticationCodeSchema = z
  .string()
  .describe("The 6-digit authentication code that the user received");

const implementationAgent = new Agent<Context>({
  name: "Implementation agent",
  description: "Helps users implement purchased solutions",
  instructions:
    "You are an agent that is helping a user to implement the {{product}} solution. Answer their " +
...
const paymentCollector = new Agent<Context>({
  ...
  description:
    "Collects payment information to charge the user for the {{product}}",
  instructions:
    "You are a helpful payment processor whose job it is to collect payment information from a user " +
  ...
  tools: {
    validate_bank_account_information: {
      type: "function",
      description:
        "Call this tool to validate bank account information before executing a charge, to avoid payment fraud.",
      ...
      execute: async (information) => {
        ...
        return {
          result:
            num >= 5 ? "This account is valid" : "This account is not valid",
        };
      },
    },
    validate_card_information: {
      type: "function",
      description:
        "Call this tool to validate credit card information before executing a charge, to prevent chargebacks",
    ...
    charge_credit_card: {
      ...
      parameters: cardInformationSchema.merge(
      ...
      execute: async (cardInfo) => {
        ...
        return {
          result:
            "Transaction pending. Please submit the 6-digit authentication code that has been sent " +
    ...
    charge_bank_account: {
      ...
      parameters: bankAccountInformationSchema.merge(
        z.object({ product: productSchema })
      ),
    ...
    submit_authentication_code: {
      type: "function",
      description:
        "Call this tool to submit a 6-digit authentication code to complete a transaction",
      parameters: z.object({
    ...
    transfer_to_implementation_agent: {
      type: "handover",
      description:
        "Call this tool to transfer the user to an agent to help them implement a solution once they " +
        "have paid for it.",
      parameters: z.object({
...
const managerAgent = new Agent<Context>({
  ...
  instructions:
    "You talk to people who have been speaking to an IBM sales agent, but who wanted to talk to" +
...
const aiSalesAgent = new Agent<Context>({
  ...
  instructions:
    "You are a sales agent on the phone whose job it is to answer questions about IBM's Watson platform. " +
  ...
  tools: {
    transfer_to_payments: {
      type: "handover",
      description:
        "Call this tool to transfer to the payment collector agent so that they can process payment",
      parameters: z.object({
        product: productSchema,
    ...
    transfer_to_manager: {
      type: "handover",
      description:
        "Call this tool to transfer the call to the manager when the person on the call asks to" +
...
const receptionistAgent = new Agent({
  ...
  instructions:
    "You are a receptionist agent whose job it is to greet people and route calls appropriately. " +
    "You can transfer calls to sales, to a manager, or to an implementation specialist.",
  tools: {
    transfer_to_sales: {
      type: "handover",
      description:
        "Call this tool to transfer the call to a sales agent who can answer questions about IBM " +
    ...
    transfer_to_manager: {
      type: "handover",
      description:
        "Call this tool to transfer the call to a manager if the person asks to talk to them",
      parameters: z.object({}),
      execute: async () => ({ agent: managerAgent }),
    },
    transfer_to_implementation: {
      type: "handover",
      description:
        "Transfer this call to an implementation agent who can help implement Watson and transcription solutions",
      parameters: z.object({
        product: productSchema,
        swarmContext: z.custom<Context>(),
      }),
...
managerAgent.tools!["transfer_to_sales"] = {
  type: "handover",
  description:
    "Call this tool to transfer back to the sales agent when the user is ready to talk to them again. ",
  parameters: z.object({
    instructions: z
      .string()
      .describe(
        "Instructions for the sales agent based on your conversation, e.g. 'be more polite'"
      ),
    swarmContext: z.custom<Context>(),
  }),
